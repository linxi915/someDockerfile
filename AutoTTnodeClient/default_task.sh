...
defaultListFile="/pss/AutoTTnodeClient/$DEFAULT_LIST_FILE"
echo "默认文件定时任务文件路径为 ${defaultListFile}"
if [ $CUSTOM_LIST_FILE ]; then
    customListFile="/pss/AutoTTnodeClient/$CUSTOM_LIST_FILE"
...
sh -x /pss/AutoTTnodeClient/scripts_update.sh

echo "第3步判断是否存在自定义定时任务列表并追加..."
if [ $CUSTOM_LIST_FILE ]; then
    echo "您配置了自定义任务文件： $CUSTOM_LIST_FILE ，自定义任务类型为： $CUSTOM_LIST_MERGE_TYPE"
    if [ -f "$customListFile" ]; then
        if [ $CUSTOM_LIST_MERGE_TYPE == "append" ]; then
            echo "合并默认定时任务文件： $DEFAULT_LIST_FILE 和自定义定时任务文件： $CUSTOM_LIST_FILE"
...
if [ $(grep -c "docker_entrypoint.sh" $mergedListFile) -eq '0' ]; then
    ...
    echo "52 */6 * * * docker_entrypoint.sh >> /logs/default_task.log 2>&1" >> $mergedListFile
else
...
echo "第7步将仓库的 docker_entrypoint.sh 脚本更新至系统 /usr/local/bin/docker_entrypoint.sh 内..."
cat /pss/AutoTTnodeClient/docker_entrypoint.sh > /usr/local/bin/docker_entrypoint.sh
