...
if [ $jd_jxFactoryCreateTuan_ENABLE = "Y" ]; then
    ...
    echo "30 * * * * cd /scripts && node jd_jxFactoryCreateTuan.js >> logs/jd_jxFactoryCreateTuan.log 2>&1 && upload" >> $mergedListFile
fi

##更新抢京豆邀请码
if [ $jd_updateBeanHome_ENABLE = "Y" ]; then
    echo "# 更新抢京豆邀请码" >> $mergedListFile
    echo "5 0,2 * * * cd /scripts && node jd_updateBeanHome.js >> logs/jd_updateBeanHome.log 2>&1 && upload" >> $mergedListFile
fi

##京东签到领现金
if [ $jd_updateCash_ENABLE = "Y" ]; then
    echo "# 京东签到领现金" >> $mergedListFile
    echo "1 0 * * * cd /scripts && node jd_updateCash.js >> logs/jd_updateCash.log 2>&1 && upload" >> $mergedListFile
fi

##京喜财富岛
if [ $jd_updateCfd_ENABLE = "Y" ]; then
    echo "# 京喜财富岛" >> $mergedListFile
    echo "3 0 * * * cd /scripts && node jd_updateCfd.js >> logs/jd_updateCfd.log 2>&1 && upload" >> $mergedListFile
fi

##更新东东小窝邀请码
if [ $jd_updateSmallHome_ENABLE = "Y" ]; then
    echo "# 更新东东小窝邀请码" >> $mergedListFile
    echo "10 0 * * * cd /scripts && node jd_updateSmallHome.js >> logs/jd_updateSmallHome.log 2>&1 && upload" >> $mergedListFile
fi
