...
if [ $jd_updateCfd_ENABLE = "Y" ]; then
    ...
    echo "0-3/1 0,7 * * * cd /scripts && node jd_updateCfd.js >> logs/jd_updateCfd.log 2>&1 && upload >> logs/upload_jd_updateCfd.log 2>&1" >> $mergedListFile
fi

##全民开红包
if [ $jd_updateRed_ENABLE = "Y" ]; then
    echo "# 全民开红包" >> $mergedListFile
    echo "0 0,4 * * * cd /scripts && node jd_updateRed.js >> logs/jd_updateRed.log 2>&1 && upload >> logs/upload_jd_updateRed.log 2>&1" >> $mergedListFile
fi
