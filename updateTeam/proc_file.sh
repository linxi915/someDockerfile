...
if [ $jd_updateBeanHome_ENABLE = "Y" ]; then
    ...
    echo "0 0,2 * * * cd /scripts && node jd_updateBeanHome.js >> logs/jd_updateBeanHome.log 2>&1 && upload" >> $mergedListFile
fi

##京东签到领现金
if [ $jd_updateCash_ENABLE = "Y" ]; then
    echo "# 京东签到领现金" >> $mergedListFile
    echo "0 0 * * * cd /scripts && node jd_updateCash.js >> logs/jd_updateCash.log 2>&1 && upload" >> $mergedListFile
fi
...
if [ $jd_updateSmallHome_ENABLE = "Y" ]; then
    ...
    echo "0 0 * * * cd /scripts && node jd_updateSmallHome.js >> logs/jd_updateSmallHome.log 2>&1 && upload" >> $mergedListFile
fi
